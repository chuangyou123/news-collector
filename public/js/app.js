...
// ═══ News Library ═══
async function loadLibrary(){try{const r=await fetch('/api/news/all');allNews=await r.json()}catch{allNews=allNews||[]}const q=searchInput.value.trim().toLowerCase();let list=allNews;if(q)list=allNews.filter(n=>n.content.toLowerCase().includes(q));libraryList.innerHTML=list.length?list.map((n,i)=>'<div class="lib-item"><span class="lib-num">'+(i+1)+'.</span><span class="lib-user">'+esc(n.username)+'</span><span class="lib-text">'+esc((n.content||'').slice(0,120))+(n.content&&n.content.length>120?'...':'')+'</span>'+(adminIPOk&&!n.id.startsWith('seed-')?'<button class="lib-del" onclick="delFromLib(\''+n.id+'\')">x</button>':'')+'</div>').join(''):'<div class="empty-state"><p>empty</p></div>';totalCount.textContent=list.length;}
...
// ═══ Direct Delete ═══
window.delFromLib=async function(id){if(!confirm('delete?'))return;if(!adminKey){showToast('need key','error');return}try{const r=await fetch('/api/admin/news/'+id,{method:'DELETE',headers:adminHeaders()});if(!r.ok){const d=await r.json();throw new Error(d.error)}showToast('deleted','success')}catch(e){showToast(e.message,'error')}};
...
adminKeyConfirm.addEventListener('click',async()=>{const k=adminKeyInput.value.trim();if(!k){showToast('input key','error');return}adminKeyConfirm.disabled=true;try{const r=await fetch('/api/admin/auth',{method:'POST',headers:{'Content-Type':'application/json',Authorization:authToken||''},body:JSON.stringify({key:k})});if(!r.ok)throw new Error('wrong key');adminKey=k;adminKeyOverlay.style.display='none';adminKeyInput.value='';openAdminPanel()}catch(e){showToast(e.message,'error')}finally{adminKeyConfirm.disabled=false;}});
...
async function loadAdminNews(){const d=adminDateFilter.value||new Date().toISOString().slice(0,10);try{const r=await fetch('/api/admin/news?date='+d,{headers:adminHeaders()});if(!r.ok)throw new Error('err');const list=await r.json();adminNewsList.innerHTML=list.length?list.map(n=>'<div class="admin-news-item"><div class="admin-news-info"><span class="admin-news-user">'+esc(n.username)+'</span><span class="admin-news-title">'+esc((n.content||'').slice(0,50))+'</span><span class="admin-news-time">'+fmt(n.time||n.created_at)+'</span></div><div class="admin-news-actions">'+(!n.seed?'<button class="btn-sm '+(n.pinned?'btn-unpin':'btn-pin')+'" onclick="togglePin(\''+n.id+'\','+!n.pinned+')">'+(n.pinned?'unpin':'pin')+'</button><button class="btn-del" onclick="deleteNews(\''+n.id+'\')">del</button>':'')+'</div></div>').join(''):'<div class="empty-state small"><p>empty</p></div>';}catch(e){adminNewsList.innerHTML='<div class="empty-state small"><p>'+esc(e.message)+'</p></div>';}}

window.togglePin=async(id,pin)=>{try{const r=await fetch('/api/admin/pin',{method:'POST',headers:{'Content-Type':'application/json',...adminHeaders()},body:JSON.stringify({id,pinned:pin})});if(!r.ok)throw new Error((await r.json()).error);showToast(pin?'pinned':'unpinned','success');loadAdminNews()}catch(e){showToast(e.message,'error')}};
window.deleteNews=async(id)=>{if(!confirm('delete?'))return;try{const r=await fetch('/api/admin/news/'+id,{method:'DELETE',headers:adminHeaders()});if(!r.ok)throw new Error((await r.json()).error);showToast('deleted','success');loadAdminNews()}catch(e){showToast(e.message,'error')}};

adminCopyToday.addEventListener('click',async()=>{const d=adminDateFilter.value;try{const url=d?'/api/admin/news?date='+d:'/api/news/all';const r=await fetch(url,{headers:adminHeaders()});const list=await r.json();if(!list.length){showToast('empty','info');return}await navigator.clipboard.writeText(list.map(x=>x.content).join(','));showToast('copied '+list.length,'success')}catch(e){showToast('fail','error')}});

async function loadAdminUsers(){try{const r=await fetch('/api/admin/users',{headers:adminHeaders()});if(!r.ok)throw new Error('err');const list=await r.json();adminUserList.innerHTML=list.length?list.map(u=>'<div class="admin-user-item '+(u.banned?'banned':'')+'"><div class="admin-user-info"><span class="admin-user-name">'+esc(u.username)+(u.banned?' banned':'')+(u.role==='superadmin'?' SUPER':u.role==='admin'?' ADMIN':'')+'</span><span class="admin-user-ip">IP:'+esc(u.ip)+'</span><span class="admin-user-count">'+u.count+'</span><span class="admin-user-role">'+(u.role==='superadmin'?'super':u.role==='admin'?'admin':'user')+'</span></div><div class="admin-user-actions">'+(u.role!=='superadmin'?'<button class="btn-sm '+(u.role==='admin'?'btn-demote':'btn-promote')+'" onclick="toggleRole(\''+esc(u.username)+'\',\''+(u.role==='admin'?'user':'admin')+'\')">'+(u.role==='admin'?'demote':'promote')+'</button>':'')+'<button class="btn-sm btn-warn" onclick="warnUser(\''+esc(u.username)+'\')">warn</button><button class="btn-sm '+(u.banned?'btn-unban':'btn-ban')+'" onclick="toggleBan(\''+esc(u.username)+'\','+!u.banned+')">'+(u.banned?'unban':'ban')+'</button><button class="btn-sm btn-pwd" onclick="openPwd(\''+esc(u.username)+'\')">pwd</button></div></div>').join(''):'<div class="empty-state small"><p>empty</p></div>';}catch(e){adminUserList.innerHTML='<div class="empty-state small"><p>'+esc(e.message)+'</p></div>';}}

window.toggleBan=async(u,ban)=>{const h=ban?prompt('hours (0=forever):','0'):'0';if(ban&&h===null)return;if(!confirm((ban?'ban':'unban')+' '+u+'?'+(ban&&parseInt(h)>0?' '+h+'h':'')))return;try{const r=await fetch('/api/admin/ban',{method:'POST',headers:{'Content-Type':'application/json',...adminHeaders()},body:JSON.stringify({username:u,banned:ban,duration:parseInt(h)||0})});if(!r.ok)throw new Error((await r.json()).error);showToast('ok','success');loadAdminUsers()}catch(e){showToast(e.message,'error')}};
window.toggleRole=async(u,role)=>{if(!confirm((role==='admin'?'promote':'demote')+' '+u+'?'))return;try{const r=await fetch('/api/admin/set-role',{method:'POST',headers:{'Content-Type':'application/json',...adminHeaders(),'x-user-token':authToken},body:JSON.stringify({username:u,role})});if(!r.ok)throw new Error((await r.json()).error);showToast('ok','success');loadAdminUsers()}catch(e){showToast(e.message,'error')}};
...
pwdModalConfirm.addEventListener('click',async()=>{const p=pwdModalInput.value.trim();if(!p||p.length<4){showToast('min 4 chars','error');return}try{const r=await fetch('/api/admin/reset-password',{method:'POST',headers:{'Content-Type':'application/json',...adminHeaders()},body:JSON.stringify({username:pwdModalUsername,newPassword:p})});if(!r.ok)throw new Error((await r.json()).error);showToast('pwd changed','success');pwdModalOverlay.style.display='none'}catch(e){showToast(e.message,'error')}});

// ═══ Announce & Warn ═══
window.postAnnounce=async()=>{const c=announceInput.value.trim();if(!c){showToast('empty','error');return}try{const r=await fetch('/api/admin/announcement',{method:'POST',headers:{'Content-Type':'application/json',...adminHeaders(),'x-user-token':authToken},body:JSON.stringify({content:c})});if(!r.ok)throw new Error((await r.json()).error);announceInput.value='';showToast('posted','success');loadAdminAnnouncements()}catch(e){showToast(e.message,'error')}};
