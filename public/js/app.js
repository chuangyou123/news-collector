...
//  新闻收集站 — 三Tab版 v2
// ═══════════════════════════════════════════════
// SW disabled to avoid cache issues
...
function onAuthSuccess(data){currentUser=data.username;authToken=data.token;localStorage.setItem('nc_user',data.username);localStorage.setItem('nc_token',data.token);socket.emit('login',data.username);userArea.style.display='none';userInfo.style.display='flex';userBadge.textContent=data.username;userBadge.style.cursor='pointer';userBadge.title='点击查看个人主页';userBadge.onclick=()=>showProfile(data.username);uploadCard.style.display='block';usernameInput.value='';passwordInput.value='';checkAdminIP();}
...
function renderLibrary(){const q=searchInput.value.trim().toLowerCase();let list=libAll;if(q)list=libAll.filter(n=>n.content.toLowerCase().includes(q));const perPage=30;const tp=Math.ceil(list.length/perPage);const cp=Math.min(libPage+1,tp);const start=(cp-1)*perPage;const chunk=list.slice(start,start+perPage);libraryList.innerHTML=chunk.length?chunk.map((n,i)=>'<div class="lib-item"><span class="lib-num">'+(start+i+1)+'.</span><span class="lib-user">'+esc(n.username)+'</span><span class="lib-text">'+esc((n.content||'').slice(0,120))+(n.content&&n.content.length>120?'...':'')+'</span>'+(adminIPOk&&!n.id.startsWith('seed-')?'<button class="lib-del" onclick="delFromLib(\''+n.id+'\')">x</button>':'')+'</div>').join(''):'<div class="empty-state"><p>empty</p></div>';let pg='';const r=2;for(let i=1;i<=tp;i++){if(i===1||i===tp||(i>=cp-r&&i<=cp+r))pg+='<button class="page-btn'+(i===cp?' active':'')+'" onclick="libPage='+(i-1)+';renderLibrary()">'+i+'</button>';else if(i===cp-r-1||i===cp+r+1)pg+='<span style=\"padding:0 4px\">...</span>'}libraryList.innerHTML+='<div class="pagination">'+pg+'</div>';totalCount.textContent=libAll.length;}
...
window.showProfile=async function(username){
  ...
  const achLen=(u.achievements||[]).length;
  let achHtml='';
  if(achLen>0){achHtml='<div style="margin-top:8px"><b>🏅 成就</b><div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:4px">'+u.achievements.map(a=>'<span style="background:#FEF3C7;color:#D97706;padding:2px 8px;border-radius:10px;font-size:.75rem">'+esc(a.badge)+'</span>').join('')+'</div></div>'}
  const roleBadge=role==='superadmin'?' 👑':role==='admin'?' 🔧':'';
  const onlineBadge=u.online?' <span style="color:#10B981;font-size:.7rem">●在线</span>':'';
  const mySection=u.username===currentUser?'<div style="margin-bottom:8px"><input id="sigInput" style="width:100%;padding:6px;border:1px solid #ddd;border-radius:6px" placeholder="修改签名..."><button class="btn-sm" style="margin-top:4px" onclick="updateSignature()">保存签名</button></div>':'<button class="btn-sm" style="margin-bottom:8px" onclick="followUser('+JSON.stringify(u.username)+')">➕ 关注</button>';
  d.innerHTML='<div style="background:#fff;border-radius:16px;padding:24px;max-width:400px;width:90%"><div style="display:flex;align-items:center;gap:12px;margin-bottom:16px"><div style="width:50px;height:50px;border-radius:50%;background:'+(u.avatar||'#54A0FF')+';display:flex;align-items:center;justify-content:center;color:#fff;font-size:1.3rem;font-weight:700">'+u.username[0].toUpperCase()+'</div><div><div style="font-weight:700;font-size:1.1rem">'+esc(u.username)+onlineBadge+roleBadge+'</div><div style="color:#888;font-size:.8rem">'+esc(sig)+'</div></div></div>'+mySection+'<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px"><div style="text-align:center;padding:8px;background:#f5f5f5;border-radius:8px"><div style="font-weight:700">'+u.newsCount+'</div><div style="font-size:.75rem;color:#888">新闻</div></div><div style="text-align:center;padding:8px;background:#f5f5f5;border-radius:8px"><div style="font-weight:700">'+achLen+'</div><div style="font-size:.75rem;color:#888">成就</div></div></div>'+achHtml+'</div>';
...
function openAdminPanel(){adminOverlay.style.display='flex';adminDateFilter.value=new Date().toISOString().slice(0,10);setupStatsTab();switchAdminTab('news');loadAdminNews();adminKeyInput.blur();}
