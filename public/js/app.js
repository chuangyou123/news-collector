...
async function loadAdminUsers(){try{const r=await fetch('/api/admin/users',{headers:adminHeaders()});if(!r.ok)throw new Error('无权限');const list=await r.json();adminUserList.innerHTML=list.length?list.map(u=>`<div class=\"admin-user-item ${u.banned?'banned':''}\"><div class=\"admin-user-info\"><span class=\"admin-user-name\">${esc(u.username)}${u.banned?'🚫':''}${u.role==='superadmin'?'👑':u.role==='admin'?'🔧':''}</span><span class=\"admin-user-ip\">IP:${esc(u.ip)}</span><span class=\"admin-user-count\">📰${u.count}条</span><span class=\"admin-user-role\">${u.role==='superadmin'?'超管':u.role==='admin'?'管理':'用户'}</span></div><div class=\"admin-user-actions\">${u.role!=='superadmin'?`<button class=\"btn-sm ${u.role==='admin'?'btn-demote':'btn-promote'}\" onclick=\"toggleRole('${esc(u.username)}','${u.role==='admin'?'user':'admin'}')\">${u.role==='admin'?'⬇撤销':'⬆设为管理'}</button>`:''}<button class=\"btn-sm btn-warn\" onclick=\"warnUser('${esc(u.username)}')\">⚠️警告</button><button class=\"btn-sm ${u.banned?'btn-unban':'btn-ban'}\" onclick=\"toggleBan('${esc(u.username)}',${!u.banned})\">${u.banned?'✅解封':'🚫封禁'}</button><button class=\"btn-sm btn-pwd\" onclick=\"openPwd('${esc(u.username)}')\">🔑改密</button></div></div>`).join(''):'<div class=\"empty-state small\"><p>暂无</p></div>';}catch(e){adminUserList.innerHTML='<div class=\"empty-state small\"><p>'+esc(e.message)+'</p></div>';}}
window.toggleBan=async(u,ban)=>{const h=ban?prompt('封禁时长（小时，0=永久，留空=永久）：','0'):'0';if(ban&&h===null)return;if(!confirm('确定'+(ban?'封禁':'解封')+'「'+u+'」？'+(ban&&parseInt(h)>0?'时长：'+h+'小时':'')))return;try{const r=await fetch('/api/admin/ban',{method:'POST',headers:{'Content-Type':'application/json',...adminHeaders()},body:JSON.stringify({username:u,banned:ban,duration:parseInt(h)||0})});if(!r.ok)throw new Error((await r.json()).error);showToast('成功','success');loadAdminUsers()}catch(e){showToast('❌'+e.message,'error')}};
