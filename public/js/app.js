...
function renderLibrary(){const q=searchInput.value.trim().toLowerCase();let list=libAll;if(q)list=libAll.filter(n=>n.content.toLowerCase().includes(q));const chunk=list.slice(0,(libPage+1)*100);libraryList.innerHTML=chunk.length?chunk.map((n,i)=>'<div class="lib-item"><span class="lib-num">'+(i+1)+'.</span><span class="lib-user">'+esc(n.username)+'</span><span class="lib-text">'+esc((n.content||'').slice(0,120))+(n.content&&n.content.length>120?'...':'')+'</span>'+(adminIPOk&&!n.id.startsWith('seed-')?'<button class="lib-del" onclick="delFromLib(\''+n.id+'\')">x</button>':'')+'</div>').join(''):'<div class="empty-state"><p>empty</p></div>';if(chunk.length<list.length)libraryList.innerHTML+='<div style=\"text-align:center;padding:10px\"><button class=\"btn-primary\" onclick=\"libPage++;renderLibrary()\">加载更多 ('+(list.length-chunk.length)+'条)</button></div>';totalCount.textContent=libAll.length;}
...
async function loadAdminStats(){try{const r=await fetch('/api/admin/stats',{headers:adminHeaders()});const d=await r.json();const s=$('#tabStats')||(()=>{const div=document.createElement('div');div.id='tabStats';div.className='admin-tab-content';div.style.display='none';document.querySelector('.admin-panel').appendChild(div);const btn=document.createElement('button');btn.className='admin-tab';btn.dataset.tab='stats';btn.textContent='📊 统计';btn.onclick=()=>{document.querySelectorAll('.admin-tab').forEach(x=>x.classList.remove('active'));btn.classList.add('active');switchAdminTab('stats')};document.querySelector('.admin-tabs').appendChild(btn);return div})();s.innerHTML='<div style=\"padding:16px\"><h3>📊 站点统计</h3><div style=\"display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px\"><div style=\"background:#EEF2FF;padding:16px;border-radius:8px;text-align:center\"><div style=\"font-size:2rem;font-weight:700;color:#4F46E5\">'+d.users+'</div><div>注册用户</div></div><div style=\"background:#FEF3C7;padding:16px;border-radius:8px;text-align:center\"><div style=\"font-size:2rem;font-weight:700;color:#D97706\">'+d.news+'</div><div>总新闻</div></div><div style=\"background:#D1FAE5;padding:16px;border-radius:8px;text-align:center\"><div style=\"font-size:2rem;font-weight:700;color:#059669\">'+d.today+'</div><div>今日新闻</div></div><div style=\"background:#FCE7F3;padding:16px;border-radius:8px;text-align:center\"><div style=\"font-size:2rem;font-weight:700;color:#DB2777\">'+d.likes+'</div><div>总点赞</div></div></div>';if(d.daily&&d.daily.length){const m=Math.max(...d.daily.map(x=>parseInt(x.count)),1);s.innerHTML+='<div style="margin-top:16px"><h4>📈 7日趋势</h4><div style="display:flex;align-items:flex-end;gap:4px;height:80px">'+d.daily.map(day=>'<div style="flex:1;text-align:center"><div style="font-size:.65rem;color:#888">'+day.count+'</div><div style="background:linear-gradient(#4F46E5,#818CF8);height:'+Math.round(parseInt(day.count)/m*70)+'px;border-radius:3px 3px 0 0"></div><div style="font-size:.55rem;color:#888;margin-top:2px">'+(day.date||'').slice(5)+'</div></div>').join('')+'</div></div>'}s.innerHTML+='</div>'}catch(e){}}

async function loadAdminUsers(){try{const r=await fetch('/api/admin/users',{headers:adminHeaders()});if(!r.ok)throw new Error('err');const list=await r.json();adminUserList.innerHTML=list.length?list.map(u=>'<div class="admin-user-item '+(u.banned?'banned':'')+'"><div class="admin-user-info"><span class="admin-user-name">'+esc(u.username)+(u.banned?' 已封':'')+(u.role==='superadmin'?' 👑超管':u.role==='admin'?' 🔧管理':'')+'</span><span class="admin-user-ip">IP:'+esc(u.ip)+'</span><span class="admin-user-count">'+u.count+'条</span><span class="admin-user-role">'+(u.role==='superadmin'?'超级管理员':u.role==='admin'?'管理员':'用户')+'</span></div><div class="admin-user-actions">'+(u.role!=='superadmin'?'<button class="btn-sm '+(u.role==='admin'?'btn-demote':'btn-promote')+'" onclick="toggleRole(\''+esc(u.username)+'\',\''+(u.role==='admin'?'user':'admin')+'\')">'+(u.role==='admin'?'撤销管理':'设为管理')+'</button>':'')+'<button class="btn-sm btn-warn" onclick="warnUser(\''+esc(u.username)+'\')">⚠警告</button><button class="btn-sm '+(u.banned?'btn-unban':'btn-ban')+'" onclick="toggleBan(\''+esc(u.username)+'\','+!u.banned+')">'+(u.banned?'解封':'封禁')+'</button><button class="btn-sm btn-pwd" onclick="openPwd(\''+esc(u.username)+'\')">改密</button></div></div>').join(''):'<div class="empty-state small"><p>暂无</p></div>';}catch(e){adminUserList.innerHTML='<div class="empty-state small"><p>'+esc(e.message)+'</p></div>';}}
